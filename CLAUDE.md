...
```ts
{
  ...
  otaBookings: [{ platformId, bookingsCount, roomRevenueSen, guestPaidPlatform }],
...
- **Passwords.** Argon2id via `@node-rs/argon2` (prebuilt, no native build) in `lib/password.ts`. Node runtime only. Never store or log plaintext. Strength policy (minimum length, weak-word/sequence/repeated-character/email-echo rejection) lives in `lib/passwordPolicy.ts`, checked on every owner-set password at `/settings/users`.
- **Password minimum is 8, not a bare backstop.** It used to be 12, covering both offline attacks (a stolen hash, cracked at leisure — Argon2id's job) and online ones (an attacker submitting real login attempts). Login rate limiting (below) now covers the online case, so length doesn't have to carry both; the weak-password rejection list in `lib/passwordPolicy.ts` is unchanged.
- **Login rate limiting** (`lib/loginRateLimit.ts` — pure schedule, unit-tested; `lib/loginRateLimitStore.ts` — DB access) tracks failed attempts in a new `loginAttempts` collection, one document per `{keyType: "email"|"ip", key}` (unique compound index). Either the attempted email or the source IP being locked blocks the attempt **before** `getUserByEmail`/`verifyPassword` run — no wasted Argon2 work, no timing signal. 5 failures triggers a 1-minute lockout; every additional 5 escalates (5 min, 15 min, 60 min cap). A successful login resets only the *email* counter, never the IP one — otherwise an attacker spraying many emails from one IP could undo their own lockout the moment one guess lands. Every failed attempt (including one rejected purely for being locked out) is audit-logged (`action: "login_failed"`, collection `"loginAttempts"`) — never the password.
...
## OTA platforms and remittances

Replaces the single aggregate `collections.otaPrepaidSen` field with per-platform tracking: which OTA a booking came through, who collected the money, and — on a new manager+ `/ota` page — what's still owed and a remittance-recording flow.

- **`otaPlatforms`** (`lib/otaPlatforms.ts`/`otaPlatformsStore.ts`) is a managed list — `name`, `active`, `displayOrder`, `guestPaysPlatform` (the platform's default for a new booking line) — same shape and Settings pattern as `paymentMethods` (`/settings/ota-platforms`, manager+, never hard-deleted). Seeded: Agoda and Trip.com default to guest-pays-platform; Booking.com, Tiket.com and Traveloka default to guest-pays-us.
- **The night report's "OTA bookings" section** (`lib/nightReport.ts`'s `OtaBookingLineSchema`, a new `otaBookings` array on `NightReportInputSchema`/the stored document) is one line per platform with bookings that night — `platformId`, `bookingsCount` (informational, not cross-checked against `rooms.sold`), `roomRevenueSen` (the full room rate, never the expected payout — commission is recognised later, from the remittance, not by reception), and `guestPaidPlatform` (defaults from the platform, freely overridable per line). Reception adds a line only when there were bookings; an ordinary night stays empty.
- **Room revenue for an OTA booking is already inside `rooms.revenueSen`** — the section is attribution and receivable bookkeeping, not additive to `totalRevenueSen()`.
- **`otaReceivableSen()`** (`lib/nightReport.ts`) sums `roomRevenueSen` across lines where `guestPaidPlatform` is true — guest-paid-us lines contribute nothing, since that revenue is already inside whatever reception typed into `collections.cashSen`/`cardSen`/etc, so it's never double counted. This value replaces the old `otaPrepaidSen` term in `revenueGap()`'s `receivablesAddedSen`.
- **No historical `businessDays` document was migrated.** Approved days are immutable (rule 5) and were never touched. `NightDayDoc.collections.otaPrepaidSen` stays as an optional, read-only legacy field purely so `collectionsByChannel()` (`lib/reportSummary.ts`) can still bucket old months into an "OTA prepaid (legacy)" channel instead of silently losing them — no code path ever writes it again. The one place old and new schema meet is the pre-approval editor: a `submitted`-but-unapproved report from before this shipped, with no `otaBookings` and a nonzero legacy `otaPrepaidSen`, is bridged into one prefilled line with a blank `platformId` (`app/reception/edit/[id]/page.tsx`) — the schema requires a real platform to save, so the manager must pick one or delete the line before the edit can be saved; nothing is written until they do.
- **`otaRemittances`** (`lib/otaRemittances.ts`/`otaRemittancesStore.ts`) is one document per remittance a platform pays: `platformId`, `date` (a bank-side event, not a business date), `amountReceivedSen` (what actually landed), `outstandingCoveredSen` (how much of the platform's outstanding this remittance is meant to clear — defaults to the full outstanding in the UI, reducible for a known-partial remittance), `paymentMethodId`, `reference`, `note`. A single `insertOne` + audit — no Mongo transaction, unlike `setShares()`: nothing else is written in the same operation, since the optional commission expense is a separate, explicitly-confirmed follow-up action (`app/ota/actions.ts`'s `recordOtaCommissionExpense`, posting through the existing `createExpense()` against the already-seeded "OTA commission" category), never auto-posted.
- **`/ota` (manager+, same tier as `/revenue`/`/expenses`)** shows, per platform: bookings and revenue booked (scoped to the page's date-range filter, same `ReportsPicker` component `/reports` uses — generalized with a `basePath` prop rather than duplicated) and received/outstanding (an **all-time running balance**, unaffected by the date filter — same precedent as the partner balance screen: `outstandingSen = Σ receivable-added (guestPaidPlatform lines, all history) − Σ outstandingCoveredSen`). The aggregation is pure and tested (`lib/otaSummary.ts`/`otaSummary.test.ts`).
- **The commission prompt**: after a remittance where `outstandingCoveredSen > amountReceivedSen`, the client shows the shortfall and offers to record it as an expense — editable, never automatic. `commissionShortfallSen()` (`lib/otaSummary.ts`) is the pure calculation, clamped at zero (an overpayment isn't a negative commission).
