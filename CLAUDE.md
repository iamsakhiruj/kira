...
- **Sidebar nav badge is real data** (`getPendingBusinessDays().length` on the Front desk item), fetched inside `AppShell` for manager/owner only. **Dashboard numbers are not** — Revenue/Expenses/Net profit/Cash show `"—"` (`app/dashboard/page.tsx`) because those collections don't exist until steps 2.3+; inventing a placeholder figure would be worse than an honest gap. Approval and missing-report counts on the dashboard *are* real, reusing the same queries as Front desk's badge and missing-days prompt.
- **Stub routes** (`/revenue`, `/expenses`, `/employees`, `/reports` — manager+; `/salary`, `/partners` — owner-only) exist now, correctly gated, with a "not built yet" message (`components/stub-page.tsx`) — so `proxy.ts`'s route map and each layout's `requireUser()` call are both already right when 2.3 onward fill them in, rather than being retrofitted later.
